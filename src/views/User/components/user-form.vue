      ...
      >
        <div class="left">
          ...
          <el-form-item label="地址" prop="addr">
            <el-input v-model.number="ruleForm.addr" placeholder="请输入地址" />
          </el-form-item>
        </div>
        <div class="right">
...
async function submit() {
  //对象解构
  const { name, addr, age, birth, sex } = ruleForm.value

  //格式化日期
  const birthDate = date.toLocaleDateString(birth)
  ...
  ruleFormRef.value.validate(async (volid) => {
    ...
    if (volid) {
      //添加用户
      await userStore.fetchAddUser(ruleForm.value)
      //更新数据，重新进行渲染
      await userStore.fetchGetUser(page.value).then((res) => {
        console.log(res)
        console.log('userF', userList.value)
      })
